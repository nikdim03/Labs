...
void remove_element_after(Node* cur, Node* next) {
  if (next->next == NULL) {
    cur->next = NULL;
    free(next);
  } else {
...
void remove_element_before(Node* cur, Node* prev) {
  if (prev->prev == NULL) {
    cur->prev = NULL;
    free(prev);
  } else {
...
void insert_after(char c, Node* cur, Node* end) {
  Node* node = (Node*)malloc(sizeof(Node));
  if (node == NULL) {
    printf("ERROR ALLOCATING MEMORY");
    exit(1);
...
void insert_before(char c, Node* cur, Node* head) {
  Node* node = (Node*)malloc(sizeof(Node));
  if (node == NULL) {
    printf("ERROR ALLOCATING MEMORY");
    exit(0);
...
void print_list_begin(Node* head, Node* cur) {
  while (head) {
    if (cur == head) {
      if (head->data == ' ') {
        printf("\033[3;45;32m");
...
void print_list_end(Node* end, Node* cur) {
  while (end) {
    if (cur == end) {
      if (end->data == ' ') {
        printf("\033[3;45;30m");
